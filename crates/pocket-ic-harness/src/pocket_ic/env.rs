...
/// Returns a [`PocketIcBuilder`] ready to be configured.
///
/// If the PocketIC server binary is not present, it downloads it and sets
/// the `POCKET_IC_BIN` environment variable accordingly.
///
/// To use a custom server binary, set the `POCKET_IC_BIN` environment variable
/// to point to the binary. The binary must be executable.
///
...
pub async fn init_pocket_ic() -> PocketIcBuilder {
    ...
    let status = INITIALIZATION_STATUS
        .get_or_init(|| async {
            ...
            if let Some(binary_path) = check_default_pocket_ic_binary_exist() {
                unsafe {
